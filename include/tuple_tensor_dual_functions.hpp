                           ...
                           ParamTypes... params)
{
  ...
  if (fl * fh > 0) {
    std::cout << "solve_scalar_equation: root not bracketed by input bounds." << std::endl;
  }
  ...
  if (converged) {
    ...
    while (!converged) {
      if (iterations == options.max_iter) {
        std::cout << "solve_scalar_equation failed to converge in allotted iterations." << std::endl;
        break;
      }
