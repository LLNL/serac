...
ConductionSolver::ConductionSolver(ParFiniteElementSpace &f, double kap)
   : TimeDependentOperator(f.GetTrueVSize(), 0.0), m_fe_space(f), m_M_form(nullptr), m_K_form(nullptr),
     ...
     m_M_solver(f.GetComm()), m_T_solver(f.GetComm()), m_z(height)
{
   ...
   m_M_form = new ParBilinearForm(&m_fe_space);
   m_M_form->AddDomainIntegrator(new MassIntegrator());
   ...
   m_K_form = new ParBilinearForm(&m_fe_space);
   m_K_form->AddDomainIntegrator(new DiffusionIntegrator(kappa_coef));
