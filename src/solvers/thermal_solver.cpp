...
DynamicConductionOperator::DynamicConductionOperator(std::shared_ptr < mfem::ParFiniteElementSpace > fespace,
  ...
  : mfem::TimeDependentOperator(fespace->GetTrueVSize(), 0.0), m_fespace(fespace),
    m_bc_rhs(nullptr), m_ess_bdr_coef(nullptr),
...
void DynamicConductionOperator::SetMMatrix(std::shared_ptr < mfem::HypreParMatrix > M_mat, std::shared_ptr < mfem::HypreParMatrix > M_e_mat)
{
  // Set the mass matrix
  m_M_mat = M_mat;
  m_M_e_mat = M_e_mat;
...
void DynamicConductionOperator::SetKMatrix(std::shared_ptr < mfem::HypreParMatrix > K_mat, std::shared_ptr < mfem::HypreParMatrix > K_e_mat)
{
...
void DynamicConductionOperator::SetLoadVector(std::shared_ptr < mfem::Vector > rhs)
{
  m_rhs = rhs;
}
