...
NonlinearSolidSolver::NonlinearSolidSolver(int order, mfem::ParMesh *pmesh)
  ...
  velocity.space = std::make_shared<mfem::ParFiniteElementSpace>(
      pmesh, velocity.coll.get(), pmesh->Dimension(), mfem::Ordering::byVDIM);
  ...
  *velocity.gf  = 0.0;
  ...
  displacement.space = std::make_shared<mfem::ParFiniteElementSpace>(
      pmesh, displacement.coll.get(), pmesh->Dimension(),
...
void NonlinearSolidSolver::SetDisplacementBCs(
...
void NonlinearSolidSolver::SetTractionBCs(
    mfem::Array<int> &trac_bdr, mfem::VectorCoefficient *trac_bdr_coef) {
  SetNaturalBCs(trac_bdr, trac_bdr_coef);
}

void NonlinearSolidSolver::SetHyperelasticMaterialParameters(double mu,
                                                             double K) {
  delete m_model;
...
void NonlinearSolidSolver::SetInitialState(
    mfem::VectorCoefficient &disp_state, mfem::VectorCoefficient &velo_state) {
...
void NonlinearSolidSolver::SetSolverParameters(
...
void NonlinearSolidSolver::CompleteSetup() {
  ...
  if (m_timestepper == TimestepMethod::QuasiStatic) {
    m_H_form->AddDomainIntegrator(
        new IncrementalHyperelasticIntegrator(m_model));
  } else {
  ...
  if (m_nat_bdr_vec_coef != nullptr) {
    m_H_form->AddBdrFaceIntegrator(
        new HyperelasticTractionIntegrator(*m_nat_bdr_vec_coef), m_nat_bdr);
  }
  ...
  if (m_timestepper != TimestepMethod::QuasiStatic) {
    ...
    m_S_form->AddDomainIntegrator(
        new mfem::VectorDiffusionIntegrator(*m_viscosity));
    m_S_form->Assemble(0);
    m_S_form->Finalize(0);
  }
  ...
  if (m_lin_params.prec == Preconditioner::BoomerAMG) {
    ...
    mfem::GMRESSolver *J_gmres =
        new mfem::GMRESSolver(m_state[0].space->GetComm());
    ...
    mfem::MINRESSolver *J_minres =
        new mfem::MINRESSolver(m_state[0].space->GetComm());
  ...
  if (m_timestepper == TimestepMethod::QuasiStatic) {
    ...
    m_timedep_oper = new NonlinearSolidDynamicOperator(
        m_H_form, m_S_form, m_M_form, m_ess_tdof_list, &m_newton_solver,
        m_lin_params);
...
// compute: y = H(x,p)
void NonlinearSolidQuasiStaticOperator::Mult(const mfem::Vector &k,
                                             mfem::Vector &      y) const {
...
mfem::Operator &NonlinearSolidQuasiStaticOperator::GetGradient(
...
NonlinearSolidDynamicOperator::NonlinearSolidDynamicOperator(
  ...
  // it
  m_reduced_oper = new NonlinearSolidReducedSystemOperator(
      H_form, S_form, M_form, ess_tdof_list);
  m_newton_solver->SetOperator(*m_reduced_oper);
}

void NonlinearSolidDynamicOperator::Mult(const mfem::Vector &vx,
                                         mfem::Vector &      dvx_dt) const {
...
void NonlinearSolidDynamicOperator::ImplicitSolve(const double        dt,
  ...
  mfem::Vector
      zero;  // empty vector is interpreted as zero r.h.s. by NewtonSolver
  m_newton_solver->Mult(zero, dv_dt);
  MFEM_VERIFY(m_newton_solver->GetConverged(),
...
void NonlinearSolidReducedSystemOperator::Mult(const mfem::Vector &k,
                                               mfem::Vector &      y) const {
...
mfem::Operator &NonlinearSolidReducedSystemOperator::GetGradient(
    const mfem::Vector &k) const {
  delete m_jacobian;
  // Form the gradient of the complete nonlinear operator
  mfem::SparseMatrix *localJ =
      Add(1.0, m_M_form->SpMat(), m_dt, m_S_form->SpMat());
  add(*m_v, m_dt, k, m_w);
  add(*m_x, m_dt, m_w, m_z);
