  ...
  m_mu(nullptr), m_lambda(nullptr), m_body_force(nullptr)
{
  ...
  // Initialize the displacement field data structures
  ...
  *displacement.gf = 0.0;
  ...
  m_state["displacement"] = displacement;
...
void ElasticitySolver::SetDisplacementBCs(mfem::Array<int> &disp_bdr, mfem::VectorCoefficient *disp_bdr_coef)
{
  ...
  m_state["displacement"].space->GetEssentialTrueDofs(disp_bdr, m_ess_tdof_list);
}
...
void ElasticitySolver::CompleteSetup()
{
  ...
  m_K_form = new mfem::ParBilinearForm(m_state["displacement"].space.get());
  ...
  m_l_form = new mfem::ParLinearForm(m_state["displacement"].space.get());
  ...
  if (m_nat_bdr_vec_coef != nullptr) {
    ...
    m_rhs = new mfem::HypreParVector(m_state["displacement"].space.get());
    *m_rhs = 0.0;
  }
  ...
  m_bc_rhs = new mfem::HypreParVector(m_state["displacement"].space.get());
  *m_bc_rhs = 0.0;

  // Initialize the true vector
  m_state["displacement"].gf->GetTrueDofs(m_state["displacement"].true_vec);

  if (m_lin_params.prec == Preconditioner::BoomerAMG) {
    MFEM_VERIFY(m_state["displacement"].space->GetOrdering() == mfem::Ordering::byVDIM,
    ...
    prec_amg->SetElasticityOptions(m_state["displacement"].space.get());
    m_K_prec = prec_amg;

    mfem::GMRESSolver *K_gmres = new mfem::GMRESSolver(m_state["displacement"].space->GetComm());
  ...
  // If not AMG, just MINRES with Jacobi smoothing
    ...
    mfem::MINRESSolver *K_minres = new mfem::MINRESSolver(m_state["displacement"].space->GetComm());
...
void ElasticitySolver::AdvanceTimestep(__attribute__((unused)) double &dt)
{
  ...
  m_state["displacement"].gf->SetFromTrueDofs(m_state["displacement"].true_vec);
  m_cycle += 1;
}
...
void ElasticitySolver::QuasiStaticSolve()
{
  ...
  *m_bc_rhs = *m_rhs;
  if (m_ess_bdr_vec_coef != nullptr) {
    ...
    m_state["displacement"].gf->ProjectBdrCoefficient(*m_ess_bdr_vec_coef, m_ess_bdr);
    m_state["displacement"].gf->GetTrueDofs(m_state["displacement"].true_vec);
    mfem::EliminateBC(*m_K_mat, *m_K_e_mat, m_ess_tdof_list, m_state["displacement"].true_vec, *m_bc_rhs);
  }

  m_K_solver->SetOperator(*m_K_mat);

  m_K_solver->Mult(*m_bc_rhs, m_state["displacement"].true_vec);
}
