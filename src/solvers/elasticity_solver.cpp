...
void ElasticitySolver::setBodyForce(const mfem::VectorCoefficient& force) { body_force_ = &force; }
...
void ElasticitySolver::completeSetup()
{
  ...
  if (nat_bdr_.size() > 0) {
    ...
    l_form_->Assemble();
    rhs_ = std::unique_ptr<mfem::HypreParVector>(l_form_->ParallelAssemble());
  } else {
  ...
  for (auto& bc : ess_bdr_) {
    K_e_mat_ = std::unique_ptr<mfem::HypreParMatrix>(K_mat_->EliminateRowsCols(bc.true_dofs));
  }
