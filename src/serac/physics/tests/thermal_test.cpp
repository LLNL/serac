...
int main(int argc, char*argv[])
{
  ...
  // This solver works because it uses matrix action
  ...
  */
  ...
  mfem::HyprePCG lin_solver(MPI_COMM_WORLD);
  lin_solver.SetPrintLevel(0);
  lin_solver.SetAbsTol(1.0e-12);
  lin_solver.SetMaxIter(1000);

  // This preconditioner doesn't work
  ...
  // This one does
  /*
  mfem::HypreDiagScale prec;
  lin_solver.SetPreconditioner(prec);
  */
  std::unique_ptr<mfem::HypreParMatrix> J;

  mfem_ext::StdFunctionOperator res_oper(
      fespace.TrueVSize(), [&residual](const mfem::Vector& u, mfem::Vector& r) { r = residual(u); },

      [&residual, &J](const mfem::Vector& u) -> mfem::Operator& {
        J.reset(grad(residual).GetMatrix(u));

        mfem::Vector input(4), output(4);
        input = 0.0;
        output = 0.0;
        ...
        for (int i = 0; i < 4; ++i) {
          input(i) = 1.0;
          grad(residual).Mult(input, output);
          output.Print();
          input = 0.0;
        }
  ...
  nonlin_solver.SetOperator(res_oper);
  ...
  */
  ...
  mfem::Vector res = residual(U);
  axom::fmt::print("L2 norm of residual post-solve = {}\n", res.Norml2());
