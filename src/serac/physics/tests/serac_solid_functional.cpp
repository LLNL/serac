...
template < typename T > ParameterizedBodyForce(T) -> ParameterizedBodyForce<T>;
...
void functional_parameterized_solid_test(double expected_disp_norm)
{
  ...
  solid_solver.addBodyForce(DependsOn<0>{}, ParameterizedBodyForce{[](const tensor< double, dim > & x){ return 0.0 * x; }});
