...
void functional_solid_test_buckle_ball()
{
  ...
  auto                    constraint_0 = std::make_unique<InequalityConstraint<ORDER, DIM>>(
      std::move(lset_0), std::move(friction_0), "serac_solid_" + std::to_string(0), meshTag, initial_penalty);
  ...
  std::array<double, DIM> corner_1;
  for (size_t i = 0; i < DIM; ++i) corner_1[i] = corner_0[i] + rigid_velo_0[i] * t_meet - rigid_velo_1[i] * t_meet;
  ...
  auto constraint_1 = std::make_unique<InequalityConstraint<ORDER, DIM>>(
      std::move(lset_1), std::move(friction_1), "serac_solid_" + std::to_string(1), meshTag, initial_penalty);
  ...
  seracSolid->setPressure(
      [&](auto, auto t) { return t > 0.5 * dt && t < 1.5 * dt ? loadMagnitude : -1e-5 * loadMagnitude; }, backSurface);
  // seracSolid->setTraction([&](auto, auto n, auto t) { return  t > 0.5 * dt && t < 1.5 * dt ? -loadMagnitude * n :
  // 1e-7 * loadMagnitude * n; }, backSurface);
...
void functional_solid_test_gate()
{
  ...
  // serac::Domain symSurface = serac::Domain::ofBoundaryElements(pmesh, serac::by_attr<DIM>(1)); // 4,5 with traction
  // makes a twist
  ...
  seracSolid->setTraction([&](auto, auto n, auto time) { return time > 1.1 ? -1.0e-3 * time * n : 0.0 * n; },
                          topSurface);
  ...
  seracSolid->setDisplacementBCs(
      {2}, [](const mfem::Vector&) { return 0.0; }, 0);
...
void functional_solid_test_friction_box()
{
  ...
  for (size_t i = 0; i < DIM; ++i) {
    ...
    auto constraint = std::make_unique<InequalityConstraint<ORDER, DIM>>(
        std::move(lset), std::move(friction), "serac_solid_" + std::to_string(i), meshTag, initial_penalty);
...
void functional_solid_test_nonlinear_snap_cell()
{
  ...
  seracSolid->setDisplacementBCs({1}, [](const mfem::Vector&, mfem::Vector& u) { u = 0.0; });
  seracSolid->setDisplacementBCs(
      {2, 3, 5, 6}, [](const mfem::Vector&) { return 0.0; }, 0);
...
void functional_solid_test_nonlinear_snap_chain()
{
  ...
  seracSolid->setDisplacementBCs({1}, [](const mfem::Vector&, mfem::Vector& u) { u = 0.0; });
  seracSolid->setDisplacementBCs(
      {2, 4}, [](const mfem::Vector&) { return 0.0; }, 0);
