...
mfem::Vector ContactData::truePressures() const
{
  ...
  for (size_t i{0}; i < contactPairs().size(); ++i) {
    if (contactPairs()[i].getContactOptions().enforcement == ContactEnforcement::LagrangeMultiplier) {
      ...
      contactPairs()[i].pressure().ParFESpace()->GetProlongationMatrix()->MultTranspose(contactPairs()[i].pressure(),
                                                                                        p_pair_true);
    }
  }
#endif
  ...
  for (size_t i{0}; i < contactPairs().size(); ++i) {
    if (contactPairs()[i].getContactOptions().enforcement == ContactEnforcement::LagrangeMultiplier) {
      ...
      g_pair_true.MakeRef(g_true, dof_offsets[static_cast<int>(i)],
                          dof_offsets[static_cast<int>(i) + 1] - dof_offsets[static_cast<int>(i)]);
      contactPairs()[i].pressure().ParFESpace()->GetProlongationMatrix()->MultTranspose(contactPairs()[i].gaps(),
                                                                                        g_pair_true);
    }
  }
#endif
...
mfem::Array<int> ContactData::pressureTrueDofOffsets() const
{
  ...
  for (size_t i{0}; i < contactPairs().size(); ++i) {
    dof_offsets[static_cast<int>(i + 1)] = dof_offsets[static_cast<int>(i)] + contactPairs()[i].numTruePressureDofs();
  }
  return dof_offsets;
}
  ...
  for (size_t i{0}; i < contactPairs().size(); ++i) {
    if (contactPairs()[i].getContactOptions().enforcement == ContactEnforcement::LagrangeMultiplier) {
      ...
      p_pair_true.MakeRef(const_cast<mfem::Vector&>(true_pressures), dof_offsets[static_cast<int>(i)],
                          dof_offsets[static_cast<int>(i) + 1] - dof_offsets[static_cast<int>(i)]);
      contactPairs()[i].pressure().ParFESpace()->GetRestrictionMatrix()->MultTranspose(p_pair_true,
