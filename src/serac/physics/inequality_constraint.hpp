...
struct LevelSet {
  ...
  double evaluate(const mfem::Vector& x) const {
    return x[1] - y_height;
  }
...
struct InequalityConstraint
{
  InequalityConstraint(std::string physics_name, std::string mesh_tag) :
    ...
    constraint_multiplier_(StateManager::newDual(H1<order, 1>{}, detail::addPrefix(physics_name, "constraint_multiplier"), mesh_tag)),
    constraint_penalty_(StateManager::newState(H1<order, 1>{}, detail::addPrefix(physics_name, "constraint_penalty"), mesh_tag)),
    constraint_npc_error_(StateManager::newState(H1<order, 1>{}, detail::addPrefix(physics_name, "constraint_npc_error"), mesh_tag)),
    constraint_diagonal_stiffness_(StateManager::newState(H1<order, 1>{}, detail::addPrefix(physics_name, "constraint_diagonal_stiffness"), mesh_tag))
  {
    constraint_ = 0.0;
  ...
  std::unique_ptr<mfem::HypreParMatrix> sumConstraintJacobian(const FiniteElementVector& x_current, std::unique_ptr<mfem::HypreParMatrix> J) {
    ...
    //printf("g\n");
    //auto diagHypreMat = std::make_unique<mfem::HypreParMatrix>(constraint_diagonal_stiffness_.comm(), J.GetGlobalNumRows(), J.GetRowStarts(), diagSparseMat.get());
    //printf("h\n");
    //printf("i,j\n");
    //printf("k\n");
    return J;
