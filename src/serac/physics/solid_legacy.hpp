...
/**
 * @brief Enum to save the deformation after the SolidLegacy module is destructed
 *
 */
...
/**
 * @brief Enum to denote the previous solve completed in the SolidLegacy module
 *
 */
 ...
 * example 10p.
 */
class SolidLegacy : public BasePhysics {
public:
  /**
  ...
  struct SolverOptions {
     ...
     *
     */
    NewtonSolverOptions H_nonlin_options;
  ...
  /**
   * @brief Construct a new Nonlinear SolidLegacy Solver object
   *
   ...
   */
  SolidLegacy(int order, const SolverOptions& options,
   ...
   */
  SolidLegacy(const InputOptions& options, const std::string& name = "");
  ...
  /**
   * @brief Destroy the Nonlinear SolidLegacy Solver object
   */
  virtual ~SolidLegacy();
 ...
 */
template <>
struct FromInlet<serac::SolidLegacy::InputOptions> {
  /// @brief Returns created object from Inlet container
  serac::SolidLegacy::InputOptions operator()(const axom::inlet::Container& base);
};
