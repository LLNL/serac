...
namespace serac{
  ...
  struct tuple < T0, T1, T2 >{ T0 v0; T1 v1; T2 v2; };
  ...
  constexpr auto & get(tuple< T ... > & values) {
    ...
    if constexpr (i == 2) { return values.v2; }
    if constexpr (i == 3) { return values.v3; }
    if constexpr (i == 4) { return values.v4; }
    if constexpr (i == 5) { return values.v5; }
    if constexpr (i == 6) { return values.v6; }
    if constexpr (i == 7) { return values.v7; }
  }
  ...
  constexpr auto & get(tuple< T ... > & values) { 
    ...
    if constexpr (sizeof ... (T) > 3) { if constexpr (std::is_same_v<S,decltype(values.v3)>) { return values.v3; } }
    if constexpr (sizeof ... (T) > 4) { if constexpr (std::is_same_v<S,decltype(values.v4)>) { return values.v4; } }
    if constexpr (sizeof ... (T) > 5) { if constexpr (std::is_same_v<S,decltype(values.v5)>) { return values.v5; } }
    if constexpr (sizeof ... (T) > 6) { if constexpr (std::is_same_v<S,decltype(values.v6)>) { return values.v6; } }
    if constexpr (sizeof ... (T) > 7) { if constexpr (std::is_same_v<S,decltype(values.v7)>) { return values.v7; } }
  }
