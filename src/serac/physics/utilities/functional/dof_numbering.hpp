...
struct DofNumbering {
  ...
  boundary_element_dofs(fespace.GetNFbyType(mfem::FaceType::Boundary), fespace.GetBE(0)->GetDof() * fespace.GetVDim()) {

    {
      ...
      dof_ids = 0.0;
      for (int i = 0; i < iota.Size(); i++) { iota[i] = i; }
    ...
    {
      auto face_restriction = fespace.GetFaceRestriction(mfem::ElementDofOrdering::LEXICOGRAPHIC, mfem::FaceType::Boundary, mfem::L2FaceValues::SingleValued);
...
struct GradientAssemblyLookupTables {
  ...
  GradientAssemblyLookupTables(mfem::ParFiniteElementSpace & test_fespace, 
                               mfem::ParFiniteElementSpace & trial_fespace) : 
    element_nonzero_LUT(trial_fespace.GetNE(), test_fespace.GetFE(0)->GetDof() * test_fespace.GetVDim(), trial_fespace.GetFE(0)->GetDof() * trial_fespace.GetVDim()), 
    boundary_element_nonzero_LUT(trial_fespace.GetNFbyType(mfem::FaceType::Boundary), test_fespace.GetBE(0)->GetDof() * test_fespace.GetVDim(), trial_fespace.GetBE(0)->GetDof() * trial_fespace.GetVDim()) {
    ...
    for (int e = 0; e < num_elements; e++) {
      for (int i = 0; i < test_dofs_.element_dofs.size(1); i++) {
        ...
        for (int j = 0; j < trial_dofs_.element_dofs.size(1); j++) {
          ...
          infos.push_back(elem_info{test_dof, trial_dof, i, j, e, mfem_sign(test_dof) * mfem_sign(trial_dof), on_boundary});
        }
      }
    }
