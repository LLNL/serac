      ...
      : J_(J), X_(X)
  {
	...
	std::cout << "gpu_policy\n";
      evaluation_ = [=](const mfem::Vector& U, mfem::Vector& R) {
        evaluation_kernel_cuda<geometry, test_space, trial_space, Q>(
            U, R, qf_derivatives.get(), J_, X_, num_elements, qf, data);
      };

      gradient_ = [=](const mfem::Vector& dU, mfem::Vector& dR) {
        gradient_kernel_cuda<geometry, test_space, trial_space, Q>(
