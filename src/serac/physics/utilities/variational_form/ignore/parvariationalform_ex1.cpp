...
int main(int argc, char* argv[])
{
  ...
  // SERAC EDIT - (auto u, auto du, auto x), u and du are unused
  auto b_coeff = [&](auto, auto, auto x) { return 4.0 * (1.0 + 2.0 * x[0] * x[0] + 2.0 * x[1] * x[1]); };

  form.AddDomainIntegrator<DomainLFIntegrator>(b_coeff);

  // SERAC EDIT - (auto u, auto du, auto x), du and x are unused
  auto a_coeff = [&](auto u, auto, auto) { return 1.0 + u; };
