...
std::unordered_map<std::string, std::string> defineAndParse(int argc, char* argv[], std::string app_description)
{
  ...
  bool print_unused_entries_input_file{false};
  app.add_flag("-u, --print-unused-entries-input-file", print_unused_entries_input_file,
  ...
  if (version) {
    ...
    if (print_unused_entries_input_file) {
      cli_opts.insert({"print-unused-entries-input-file", {}});
    }
    if (output_directory == "") {
