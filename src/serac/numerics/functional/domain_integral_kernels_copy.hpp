...
template <int index_to_differentiate, typename derivative_type, typename lambda, int dim, int n, typename... T,
          int... I>
auto batch_apply_qf_with_AD(derivative_type* qf_derivatives, lambda qf, const tensor<double, dim, n> x, Nothing*,
                            const tuple<tensor<T, n>...> inputs, std::integer_sequence<int, I...>)
{
...
template <int index_to_differentiate, typename derivative_type, typename lambda, int dim, int n, typename qpt_data_type,
          typename... T, int... I>
auto batch_apply_qf_with_AD(derivative_type* qf_derivatives, lambda qf, const tensor<double, dim, n> x,
                            qpt_data_type* qpt_data, const tuple<tensor<T, n>...> inputs,
                            std::integer_sequence<int, I...>)
{
  using return_type =
      decltype(qf(tensor<double, dim>{}, qpt_data[0], promote_to_dual_when<I == index_to_differentiate>(T{})...));
  using value_type = decltype(get_value(return_type{}));
  tensor<value_type, n> outputs{};
  for (int i = 0; i < n; i++) {
    tensor<double, dim> x_q;
    ...
    auto qdata = qpt_data[i];
    auto outputs_and_derivatives =
        qf(x_q, qdata, promote_to_dual_when<I == index_to_differentiate>(get<I>(inputs)[i])...);
    outputs[i]        = get_value(outputs_and_derivatives);
    qf_derivatives[i] = get_gradient(outputs_and_derivatives);
  }
  return outputs;
    ...
    for (int i = 0; i < test_element::ndof; i++) {
      if constexpr (g == Geometry::Quadrilateral) {
        ...
        for (int q = 0; q < Q * Q; q++) {
          ...
          source_and_flux(q) =
              serac::tuple{get<0>(get<0>(derivatives(q))) * phi, dot(dphi_dxi, get<1>(get<1>(derivatives(q))))};
        }
      }
      ...
      if constexpr (g == Geometry::Hexahedron) {
        ...
        for (int q = 0; q < nquad; q++) {
          ...
          tensor<double, dim> dphi_dxi = {G(qx, ix) * B(qy, iy) * B(qz, iz), B(qx, ix) * G(qy, iy) * B(qz, iz),
...
double transpose_first_two(double x) { return x; }
