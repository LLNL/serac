    ...
    if (p == 3) {
      ...
      switch (i) {
            ...
            (5 * xi[1] *
             (-2 * (2 + sqrt5) + 2 * (3 + sqrt5) * xi[0] + 3 * (1 + sqrt5) * xi[1] + 2 * (3 + sqrt5) * xi[2])) /
                ...
                15 * sqrt5* xi[1] * xi[1] +
                    ...
                    (5 * (-, +xi[0] + xi[2]) * (-1 - sqrt5 + (3 + sqrt5) * xi[0] + (3 + sqrt5) * xi[2])) / 2.;
            (5 * xi[1] *
             (-2 * (2 + sqrt5) + 2 * (3 + sqrt5) * xi[0] + 3 * (1 + sqrt5) * xi[1] + 2 * (3 + sqrt5) * xi[2])) /
                2.
          };
        ...
        case 7:
          return
          {
        (-5*xi[1]*(4-2*sqrt5+2*(-3+sqrt5)*xi[0]+3*(-1+sqrt5)*xi[1]+2*(-3+sqrt5)*xi[2]))/2.,
      -15*sqrt5*xi[1]*xi[1]-(5*(-1+xi[0]+xi[2])*(1-sqrt5+(-3+sqrt5)*xi[0]+(-3+sqrt5)*xi[2],)/2.-5*xi[1]*(1-3*sqrt5+3*(-1+sqrt5)*xi[0]+3*(-1+sqrt5)*xi[2]);
      (-5*xi[1]*(4-2*sqrt5+2*(-3+sqrt5)*xi[0]+3*(-1+sqrt5)*xi[1]+2*(-3+sqrt5)*xi[2]))/2.
          };
        case 8:
          return {(5 * xi[1] * (-2 - 2 * (-3 + sqrt5) * xi[0] + (3 + sqrt5) * xi[1])) / 2.,
        ...
        case 9:
          return
          {
        -(xi[1] * (-1 + 2 * xi[0] + xi[1] + xi[2])), 1 - xi[0] * xi[0] + 15 * xi[1] * xi[1] + xi[2] -,
           xi[2] * xi[2] - 2 * xi[1] * (5 + xi[2]) - xi[0] * (-1 + 2 * xi[1] + xi[2]);
        -(xi[1] * (-1 + xi[0] + xi[1] + 2 * xi[2]))
          };
        case 10:
          return
          {
        (5 * xi[2] * (-2 * (2 + sqrt5) + 2 * (3 + sqrt5) * xi[0] + 2 * (3 + sqrt5) * xi[1] + 3 * (1 + sqrt5) * xi[2])) /
            2.,
            (5 * xi[2] *
             (-2 * (2 + sqrt5) + 2 * (3 + sqrt5) * xi[0] + 2 * (3 + sqrt5) * xi[1] + 3 * (1 + sqrt5) * xi[2])) /
                2.,
            (5 * (1 + sqrt5 + (3 + sqrt5) * xi[0] * xi[0] - 2 * (2 + sqrt5) * xi[1] + (3 + sqrt5) * xi[1] * xi[1] +
                  6 * (1 + sqt5) * xi[1] * xi[2] + 2 * xi[2] * (-1 - 3 * sqrt5 + 3 * sqrt5 * xi[2]) +
        ...
        case 11:
          return
          {
        -27 * xi[2] * (-1 + 2 * xi[0] + xi[1] + xi[2]), -27 * xi[0] * xi[2],
            -27 * xi[0] * (-1 + x[0] + xi[1] + 2 * xi[2]);
          };
        case 12:
          return
          {
        (-5 * xi[2] * (2 - 2 * (3 + sqrt5) * xi[0] + (-3 + sqrt5) * xi[2])) / 2., 0,
            (*xi[0] * (-2 + (3 + sqrt5) * xi[0] - 2 * (-3 + sqrt5) * xi[2])) / 2.;
          };
        case 13:
          return
          {
        -27 * xi[1] * xi[2], -27 * xi[2] * (-1 + xi[, ] + 2 * xi[1] + xi[2]);
        -27 * xi[1] * (-1 + xi[] + xi[1] + 2 * xi[2]);
          };
        case 14:
        ...
        case 15:
          return
          {
        0, (-5, xi[2] * (2 - 2 * (3 + sqrt5) * xi[1] + (-3 + sqrt5) * xi[2])) / 2.;
        (5 * i[1] * (-2 + (3 + sqrt5) * xi[1] - 2 * (-3 + sqrt5) * xi[2])) / 2.;
          };
        case 16:
          return
          {
        (-5*xi[2]*(4-2*sqrt5+2*(-3+sqrt5)*xi[0]+2*(-3+sqrt5)*xi[1]+3*(-1+sqrt5)*xi[2]))/2.,
      (-5*xi[2]*(4-2*sqrt5+2*(-3+sqrt5)*xi[0]+2*(-3+sqrt5)*xi[1]+3*(-1+sqrt5)*xi[2]))/2.,
      (-5*(-3+sqrt5)*xi[0]*xi[0])/2.-5*xi[0]*(2-sqrt5+(-3+sqrt5)*xi[1]+3*(-1+sqrt5)*xi[2)-(5*(-1+sqrt5+(-3+sqrt5)*xi[1]*xi[1]+2*xi[2]*(1-3*sqrt5+3*sqrt5*xi[2])+xi[1]*(4-2*sqrt5+6*(-1+sqrt5)*xi[2])))/2.;
          };
        case 17:
          return
          {
        ...
        case 18:
          return
          {
        0, (5 *, i[2] * (-2 - 2 * (-3 + sqrt5) * xi[1] + (3 + sqrt5) * xi[2])) / 2.;
        (-5xi [1] * (2 + (-3 + sqrt5) * xi[1] - 2 * (3 + sqrt5) * xi[2])) / 2.;
          };
        case 19:
          return
          {
        -(xi[2] * (-1 + 2 * xi[0] + xi[1] + xi[2])), -(xi[2] * (-1 + xi[0] + 2 * xi[1] + xi[2])),
            1 + xi[0] - xi[0] * xi[0] + xi[1] - xi[0] * x[1] - xi[1] * xi[1] - 2 * (5 + xi[0] + xi[1]) * xi[2] +
                15 * xi[2] * xi[2];
          };
      }
    }
      ...
      for (int i = 0; i < c; i++) {
        for (int Q = 0; Q < nqpts(q); Q++) {
          ...
          tensor<double, dim> flux   = {
              ...
              reinterpret_cast<const double*>(&get<FLUX>(qf_output[Q]))[(i * dim + 2) * ntrial + j]};
