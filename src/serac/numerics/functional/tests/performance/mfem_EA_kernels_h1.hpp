...
static void EADiffusionAssemble2D(const int NE, const Array<double>& b, const Array<double>& g, const Vector& padata,
                                  Vector& eadata, const bool add, const int d1d = 0, const int q1d = 0)
{
...
template <int T_D1D = 0, int T_Q1D = 0>
static void EADiffusionAssemble3D(const int NE, const Array<double>& b, const Array<double>& g, const Vector& padata,
                                  Vector& eadata, const bool add, const int d1d = 0, const int q1d = 0)
{
  const int D1D = T_D1D ? T_D1D : d1d;
  const int Q1D = T_Q1D ? T_Q1D : q1d;
                ...
                for (int k1 = 0; k1 < Q1D; ++k1) {
                  for (int k2 = 0; k2 < Q1D; ++k2) {
                    for (int k3 = 0; k3 < Q1D; ++k3) {
                      ...
                      val += bbgi * D00 * bbgj + bgbi * D10 * bbgj + gbbi * D20 * bbgj + bbgi * D01 * bgbj +
                             bgbi * D11 * bgbj + gbbi * D21 * bgbj + bbgi * D02 * gbbj + bgbi * D12 * gbbj +
                             gbbi * D22 * gbbj;
                    }
                  }
                }
...
template <int T_D1D = 0, int T_Q1D = 0>
static void EAMassAssemble2D(const int NE, const Array<double>& basis, const Vector& padata, Vector& eadata,
                             const bool add, const int d1d = 0, const int q1d = 0)
{
  const int D1D = T_D1D ? T_D1D : d1d;
  const int Q1D = T_Q1D ? T_Q1D : q1d;
  MFEM_VERIFY(D1D <= MAX_D1D, "");
  ...
  MFEM_FORALL_3D(e, NE, D1D, D1D, 1, {
    ...
    {
      MFEM_FOREACH_THREAD(k2, y, Q1D) { s_D[k1][k2] = D(k1, k2, e); }
    }
    MFEM_SYNC_THREAD;
    MFEM_FOREACH_THREAD(i1, x, D1D)
    {
      MFEM_FOREACH_THREAD(i2, y, D1D)
      {
        for (int j1 = 0; j1 < D1D; ++j1) {
          for (int j2 = 0; j2 < D1D; ++j2) {
            ...
            for (int k1 = 0; k1 < Q1D; ++k1) {
              for (int k2 = 0; k2 < Q1D; ++k2) {
                val += r_B[k1][i1] * r_B[k1][j1] * r_B[k2][i2] * r_B[k2][j2] * s_D[k1][k2];
              }
            }
            if (add) {
              M(i1, i2, j1, j2, e) += val;
...
template <int T_D1D = 0, int T_Q1D = 0>
static void EAMassAssemble3D(const int NE, const Array<double>& basis, const Vector& padata, Vector& eadata,
                             const bool add, const int d1d = 0, const int q1d = 0)
{
  const int D1D = T_D1D ? T_D1D : d1d;
  const int Q1D = T_Q1D ? T_Q1D : q1d;
  ...
  MFEM_FORALL_3D(e, NE, D1D, D1D, D1D, {
    ...
    MFEM_FOREACH_THREAD(k1, x, Q1D)
    {
      ...
      {
        MFEM_FOREACH_THREAD(k3, z, Q1D) { s_D[k1][k2][k3] = D(k1, k2, k3, e); }
      }
    }
    MFEM_SYNC_THREAD;
    MFEM_FOREACH_THREAD(i1, x, D1D)
            ...
            for (int j2 = 0; j2 < D1D; ++j2) {
              for (int j3 = 0; j3 < D1D; ++j3) {
                ...
                for (int k1 = 0; k1 < Q1D; ++k1) {
                  for (int k2 = 0; k2 < Q1D; ++k2) {
                    for (int k3 = 0; k3 < Q1D; ++k3) {
                      val += r_B[k1][i1] * r_B[k1][j1] * r_B[k2][i2] * r_B[k2][j2] * r_B[k3][i3] * r_B[k3][j3] *
                             s_D[k1][k2][k3];
                    }
                  }
                }
