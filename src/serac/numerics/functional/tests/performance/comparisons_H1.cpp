...
void h1_h1_test_EA_2D(size_t num_elements, size_t num_runs)
{
...

  {
    ...
    double runtime = time([&]() {
      for (size_t i = 0; i < num_runs; i++) {
        serac::domain_integral::element_gradient_kernel_old<Geometry::Quadrilateral, test, trial, q>(
...

  {
    ...
    std::cout << "average new kernel time: " << runtime / num_runs_d << std::endl;
    auto answer = KE_1D;
    auto error  = answer_reference;
    error -= answer;
    auto relative_error = error.Norml2() / answer_reference.Norml2();
...
template <int p, int q>
void h1_h1_test_EA_3D(size_t num_elements, size_t num_runs)
{
  using serac::Geometry;
  using serac::H1;
  ...
  mfem::Vector KE_1D(static_cast<int>(num_elements * n * n * n * n * n * n));
  mfem::Vector J1D(static_cast<int>(num_elements * dim * dim * q * q * q));
  mfem::Vector rho_dv_1D(static_cast<int>(num_elements * q * q * q));
  mfem::Vector k_invJ_invJT_dv_1D(static_cast<int>(num_elements * 6 * q * q * q));
  ...
  for (size_t e = 0; e < num_elements; e++) {
    ...
    for (int i = 0; i < q * q * q; i++) {
      ...
      k_invJ_invJT_dv(i, 3, e) = k * invJ_invJT[1][1] * dv;
      k_invJ_invJT_dv(i, 4, e) = k * invJ_invJT[1][2] * dv;
      k_invJ_invJT_dv(i, 5, e) = k * invJ_invJT[2][2] * dv;
...

  {
    ...
    double runtime = time([&]() {
      for (size_t i = 0; i < num_runs; i++) {
        serac::domain_integral::element_gradient_kernel_old<Geometry::Hexahedron, test, trial, q>(
...
int main()
{
  ...
  h1_h1_test_EA_2D< 1, 2 >(num_elements, num_runs);
}
