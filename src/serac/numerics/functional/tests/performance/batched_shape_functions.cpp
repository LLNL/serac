...
void h1_h1_test(int num_elements, int num_runs)
{
...
template <int p, int q>
void hcurl_hcurl_test(int num_elements, int num_runs)
{
  using serac::Geometry;
  using serac::Hcurl;
...
int main()
{
  int num_runs     = 1;
  int num_elements = 1;
  //h1_h1_test<2 /* polynomial order */, 3 /* quadrature points / dim */>(num_elements, num_runs);

  hcurl_hcurl_test<2 /* polynomial order */, 3 /* quadrature points / dim */>(num_elements, num_runs);
}
