...
//
// camp::tuple sum1 = a + b;
// camp::tuple sum2{serac::get<0>(a) + serac::get<0>(b), serac::get<1>(a) + serac::get<1>(b)};
...
{
  return camp::tuple{(make_dual_helper<i>(serac::get<i>(args), seq))...};
}
...
SERAC_HOST_DEVICE constexpr auto make_dual_helper(const camp::tuple<T...>& args, std::integer_sequence<int, i...>)
{
  ...
  //
  // note: we use serac::make_tuple(...) instead of camp::tuple{...} here because if
  // the first argument passed in is of type `camp::tuple < camp::tuple < T ... > >`
  // then doing something like
  //
  // camp::tuple{serac::get<i>(args)...};
  //
