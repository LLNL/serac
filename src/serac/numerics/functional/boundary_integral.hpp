                   ...
                   const mfem::Vector& N, Dimension<dim>, lambda_type&& qf, std::vector<int> active_arguments)
  {
    ...
    if constexpr (exec == ExecutionSpace::CPU) {
      ...
      for_constexpr<num_active_trial_spaces>([this, num_elements, quadrature_points_per_element, &J, &X, &N, &qf,
        ...
        using derivative_type   = decltype(get_derivative_type<index, dim, trials...>(qf));
        ...
        //                            vvv
        action_of_gradient_[index] = [ptr, qf_derivatives, num_elements](const mfem::Vector& dU, mfem::Vector& dR) {
          action_of_gradient_kernel<geometry, test, which_trial_space, Q>(dU, dR, qf_derivatives, num_elements);
        };

        element_gradient_[index] = [qf_derivatives, num_elements](CPUArrayView<double, 3> K_e) {
