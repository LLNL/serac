...
namespace mfem_ext { class SecondOrderODE; }
...
class SecondOrderIntegrator : public TimeIntegrator {
  public:

  SecondOrderIntegrator(std::unique_ptr<serac::EquationSolver> solver, const serac::TimesteppingOptions& timestepping_opts) ;
...
protected:
  ...
  std::unique_ptr<serac::EquationSolver> nonlinear_solver;

  /// The timestepping options for the solid mechanics time evolution operator
  const serac::TimesteppingOptions timestepping_options;
