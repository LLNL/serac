...
void ThermalConduction::setTemperatureBCs(const std::set<int>& temp_bdr, mfem::Coefficient& temp_bdr_coef)
{
  bcs_.addEssential(temp_bdr, temp_bdr_coef, *temperature_);
}

void ThermalConduction::setFluxBCs(const std::set<int>& flux_bdr, mfem::Coefficient& flux_bdr_coef)
{
  // Set the natural (integral) boundary condition
  bcs_.addNatural(flux_bdr, flux_bdr_coef, -1);
}

void ThermalConduction::setConductivity(mfem::Coefficient& kappa)
{
  // Set the conduction coefficient
  kappa_ = &kappa;
}

void ThermalConduction::setSource(mfem::Coefficient& source)
{
