...
NonlinearSolid::NonlinearSolid(int order, std::shared_ptr<mfem::ParMesh> mesh,
    ...
    : BasePhysics(mesh, NUM_FIELDS, order),
      ...
      displacement_(*mesh,
                    FEStateOptions{.order = order, .name = "displacement"}),
  ...
  const auto& augmented_params =
      augmentAMGForElasticity(lin_params, displacement_.space());
...
NonlinearSolid::NonlinearSolid(std::shared_ptr<mfem::ParMesh> mesh,
                               const NonlinearSolid::InputInfo& info)
...
void NonlinearSolid::setDisplacementBCs(
    const std::set<int>& disp_bdr,
...
void NonlinearSolid::setTractionBCs(
    const std::set<int>& trac_bdr,
    std::shared_ptr<mfem::VectorCoefficient> trac_bdr_coef, int component) {
  bcs_.addNatural(trac_bdr, trac_bdr_coef, component);
}

void NonlinearSolid::setHyperelasticMaterialParameters(const double mu,
                                                       const double K) {
  model_ = std::make_unique<mfem::NeoHookeanModel>(mu, K);
}

void NonlinearSolid::setViscosity(
    std::unique_ptr<mfem::Coefficient>&& visc_coef) {
...
void NonlinearSolid::completeSetup() {
  ...
  K = displacement_.createOnSpace<mfem::ParNonlinearForm>();

  // Add the hyperelastic integrator
  if (timestepper_ == serac::TimestepMethod::QuasiStatic) {
    K->AddDomainIntegrator(new IncrementalHyperelasticIntegrator(model_.get()));
  } else {
    K->AddDomainIntegrator(new mfem::HyperelasticNLFIntegrator(model_.get()));
  }

  // Add the traction integrator
  for (auto& nat_bc_data : bcs_.naturals()) {
    K->AddBdrFaceIntegrator(
        new HyperelasticTractionIntegrator(nat_bc_data.vectorCoefficient()),
        nat_bc_data.markers());
  }
  ...
  if (timestepper_ == serac::TimestepMethod::QuasiStatic) {
    ...
    nonlinear_oper_ =
        std::make_unique<NonlinearSolidQuasiStaticOperator>(std::move(K), bcs_);
    ...
    //    std::move(K), std::move(C), std::move(M), bcs_, nonlin_solver_,
    //    *timedep_oper_params_);
    ...
    residual.jacobian =
      ...
      localJ->Add(c0, K->GetLocalGradient(x + u + c0 * d2u_dt2));
      J.reset(M->ParallelAssemble(localJ.get()));
      bcs_.eliminateAllEssentialDofsFromMatrix(*J);
      return *J;
    };

    ode2 = SecondOrderODE(u.Size(), [=](const double t, const double fac0,
                                        const double fac1,
                                        const mfem::Vector& displacement,
                                        const mfem::Vector& velocity,
                                        mfem::Vector& acceleration) {

      // this is is intended to be temporary
      ...
      SLIC_WARNING_IF(!nonlin_solver_.nonlinearSolver().GetConverged(),
...
void NonlinearSolid::InputInfo::defineInputFileSchema(
    axom::inlet::Table& table) {
  ...
  auto& traction_table =
      table.addTable("traction", "Cantilever tip traction vector");
  ...
  auto& solver_table =
      table.addTable("solver", "Linear and Nonlinear Solver Parameters.");
...
NonlinearSolid::InputInfo FromInlet<NonlinearSolid::InputInfo>::operator()(
  ...
  result.solver_params.H_lin_params =
      solver["linear"].get<serac::IterativeSolverParameters>();
  result.solver_params.H_nonlin_params =
      solver["nonlinear"].get<serac::NonlinearSolverParameters>();
