...
TEST(dynamic_solver, dyn_solve)
{
  ...
  auto temp_gf_coef = std::make_shared<mfem::GridFunctionCoefficient>(ts_solver.GetTemperature()->getGridFunc());
  ...
  double v_norm    = ts_solver.GetVelocity()->getGridFunc()->ComputeLpError(2.0, zerovec);
  double x_norm    = ts_solver.GetDisplacement()->getGridFunc()->ComputeLpError(2.0, zerovec);
  double temp_norm = ts_solver.GetTemperature()->getGridFunc()->ComputeLpError(2.0, zerovec);
