...
TEST(component_bc, qs_solve)
{
  ...
  StdFunctionCoefficient disp_coef([](mfem::Vector& x) { return x[0] * -1.0e-1; });
...
TEST(component_bc, qs_attribute_solve)
{
  ...
  // define the displacement vector
  StdFunctionCoefficient disp_x_coef([](mfem::Vector& x) { return x[0] * 3.0e-2; });
  StdFunctionCoefficient disp_y_coef([](mfem::Vector& x) { return x[1] * -5.0e-2; });
