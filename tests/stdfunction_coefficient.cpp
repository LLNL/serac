 ...
 protected:
  void SetUp() {
    ...
    pfes   = std::shared_ptr<ParFiniteElementSpace>(new ParFiniteElementSpace(
        pmesh.get(), new H1_FECollection(1, dim, BasisType::GaussLobatto), 1,
        Ordering::byNODES));
    pfes_v = std::shared_ptr<ParFiniteElementSpace>(new ParFiniteElementSpace(
        pmesh.get(), new H1_FECollection(1, dim, BasisType::GaussLobatto), dim,
        Ordering::byNODES));

    pfes_l2 = std::shared_ptr<ParFiniteElementSpace>(new ParFiniteElementSpace(
        pmesh.get(), new L2_FECollection(0, dim), 1, Ordering::byNODES));
  }
...
TEST_F(StdFunctionCoefficientTest, Xtest) {
  ...
  StdFunctionVectorCoefficient x_stretch(
      3, [x_mult](mfem::Vector &p, mfem::Vector &u) {
        u = p;
        u[0] *= x_mult;
      });
...
TEST_F(StdFunctionCoefficientTest, AttributeList) {
  ...
  MFEM_VERIFY(attr_list.Size() > 0 && attr_list.Sum() > 0,
  ...
  std::unique_ptr<VisItDataCollection> visit =
      std::unique_ptr<VisItDataCollection>(new VisItDataCollection(
          "StdFunctionCoefficient.AttributeModifier", pmesh.get()));
...
TEST_F(StdFunctionCoefficientTest, AttributeListSet) {
  ...
  std::unique_ptr<VisItDataCollection> visit =
      std::unique_ptr<VisItDataCollection>(new VisItDataCollection(
          "StdFunctionCoefficient.AttributeSet", pmesh.get()));
