...
TEST_F(StdFunctionCoefficientTest, Xtest)
{
  ...
  StdFunctionVectorCoefficient x_stretch(3,
  [x_mult](mfem::Vector &p, mfem::Vector &u) {
    u = p;
    u[0] *= x_mult;
  });
...
/**
In this test we assign the upper-right portion of a cube with a different attribute on-the-fly

1. Create an indicator function for the corner
2. Create an attribute list that pertains to every element of the mesh. We utilize the default "digitize" function which assigns any coefficient value > 0 to an integer attribute of 2, and otherwise 1. This can be overridden.
3. Create a coefficient of 1, and then restrict it to attribute 2. (Remember that them mesh technically only has attribute values of 1.)
4. We then use an AttributeModifierCoefficient and the attribute list which will modify the attributes on-the-fly.

 */
TEST_F(StdFunctionCoefficientTest, AttributeList)
{
  // create an indicator coefficient that paints the upper-right corner of a cube

  StdFunctionCoefficient corner([](mfem::Vector x) {
    return (x[0] > 0.75 && x[1] > 0.75) ? 1. : 0.;
  });
  ...
  /**
     Check each element in the mesh to make sure it has the corner should be a different attribute
  */
...
TEST_F(StdFunctionCoefficientTest, EssentialBC)
{
  ...
  // Check and make sure all the vertices found in the mesh that satisfy this criterion are in the attribute list
