...
{
  reader_->parseString("coef_info = { coef = function(x, y, z) return y * 2 + z end, component = 1}");
  auto& coef_table = inlet_->addTable("coef_info");
  input::CoefficientInputInfo::defineInputFileSchema(coef_table);
  auto coef_info = coef_table.get<input::CoefficientInputInfo>();
  EXPECT_EQ(coef_info.component, 1);
  EXPECT_FALSE(coef_info.isVector());
  mfem::Vector test_vec(3);
  test_vec(0)                 = 1;
  test_vec(1)                 = 2;
  test_vec(2)                 = 3;
  const auto& func            = std::get<input::CoefficientInputInfo::ScalarFunc>(coef_info.func);
  auto        expected_result = test_vec(1) * 2 + test_vec(2);
  EXPECT_FLOAT_EQ(func(test_vec), expected_result);
  EXPECT_NO_THROW(coef_info.constructScalar());
}

TEST_F(InputTest, coef_build_scalar_missing_component)
{
  reader_->parseString("coef_info = { coef = function(x, y, z) return y * 2 + z end }");
  auto& coef_table = inlet_->addTable("coef_info");
  input::CoefficientInputInfo::defineInputFileSchema(coef_table);
  EXPECT_THROW(coef_table.get<input::CoefficientInputInfo>(), SlicErrorException);
}
...
TEST_F(InputTest, coef_build_vec_from_scalar)
{
  ...
  input::CoefficientInputInfo::defineInputFileSchema(coef_table);
  auto coef_info = coef_table.get<input::CoefficientInputInfo>();
  EXPECT_THROW(coef_info.constructVector(), SlicErrorException);
}

TEST_F(InputTest, coef_build_vector)
{
  reader_->parseString("coef_info = { vec_coef = function(x, y, z) return y * 2, z, x end }");
  auto& coef_table = inlet_->addTable("coef_info");
  input::CoefficientInputInfo::defineInputFileSchema(coef_table);
  auto coef_info = coef_table.get<input::CoefficientInputInfo>();
  EXPECT_TRUE(coef_info.isVector());
  mfem::Vector test_vec(3);
  test_vec(0)       = 1;
  test_vec(1)       = 2;
  test_vec(2)       = 3;
  const auto&  func = std::get<input::CoefficientInputInfo::VecFunc>(coef_info.func);
  ...
  EXPECT_NO_THROW(coef_info.constructVector());
}
...
TEST_F(InputTest, coef_build_scalar_from_vec)
{
  ...
  input::CoefficientInputInfo::defineInputFileSchema(coef_table);
  auto coef_info = coef_table.get<input::CoefficientInputInfo>();
  EXPECT_THROW(coef_info.constructScalar(), SlicErrorException);
}
