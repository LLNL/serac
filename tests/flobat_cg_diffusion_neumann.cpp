...
TEST(cg_diffusion_solver, cg_diff_solve_neumann)
{
   ...
   CGDiffusionSolver diffusion_solver(pmesh, 1);
   ...
   dof_markers = 1;
   dof_markers[1] = 0; 
   diffusion_solver.SetNeumannBoundary(dof_markers, one);
   dof_markers = 0;
   dof_markers[1] = 1;
   diffusion_solver.SetDirichletBoundary(dof_markers, one);
