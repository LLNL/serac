...
int main(int argc, char* argv[])
{
  ...
  std::string filename = SERAC_REPO_DIR "/examples/contact/Test4.mesh";
  ...
  mfem::Vector             K_values({1.0/(3.0*(1.0 - 2.0*0.499)), 1000.0/3.0});
  ...
  // [0] (element attribute 1) : the substrate
  // [1] (element attribute 2) : indenter block
  mfem::Vector             G_values({1.0/(2.0*(1.0+0.499)), 500.0});
  ...
  std::set<int> surface_1_boundary_attributes({3});
  std::set<int> surface_2_boundary_attributes({4});
  ...
  for (int i{0}; i < static_cast<int>(n_steps); ++i) {
